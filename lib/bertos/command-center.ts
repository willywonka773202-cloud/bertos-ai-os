...
export const AGENT_ROSTER: CommandCenterAgent[] = [
  ...
  },
  {
    id: 'hermes-nous',
    name: 'Hermes Agent',
    role: 'Server-side OpenAI-compatible Hermes backend for Hostinger VPS, local Ollama-backed Hermes, or custom free endpoints.',
    category: 'local-free',
    status: 'requires-setup',
    billing: 'Free/local by default; paid model providers are optional behind Hermes.',
    bestUse: 'Manual Hermes tasks once HERMES_ENABLED, HERMES_BASE_URL, and HERMES_API_KEY are configured server-side.',
    canRunNow: 'Only after the Hermes API server is reachable through BertOS backend routes.',
    requiresApi: true,
    warning: 'Never expose the Hermes bearer token in frontend code. Use BertOS backend proxy routes only.',
    setupPrompt: 'Configure Hermes as an OpenAI-compatible API server backed by Ollama/local models, a custom compatible endpoint, or an optional free-tier/paid provider.',
  },
  {
    id: 'free-claude-code',
  ...
  },
  {
    id: 'openclaw',
    name: 'OpenClaw',
    role: 'Local/OpenClaw agent bridge, preferably configured through Ollama.',
    category: 'experimental-proxy',
    status: 'experimental',
    billing: 'Local/Ollama-backed; no paid API key required by BertOS.',
    bestUse: 'Local agent loops, messaging-gateway planning, and tool orchestration after OpenClaw onboarding.',
    canRunNow: 'Yes if openclaw is installed, onboarded, and verified by the BertOS daemon.',
    setupPrompt: 'Use OpenClaw through the BertOS local daemon only after verifying installation. Recommended setup: ollama launch openclaw --config, then npm run bertos:daemon.',
    warning: 'OpenClaw can operate powerful local tools and messaging channels. Keep destructive actions approval-gated.',
  },
  {
    id: 'browser-skills',
    name: 'Browser Skills',
...
export const PLAYBOOK_TEMPLATES: CommandCenterPlaybook[] = [
  ...
  },
  {
    ...
    prompt: 'Audit BertOS provider status and Settings provider cards. Ensure Hermes is server-side/free-local capable, FCC is experimental, Devin is external/copy-prompt only, Anti-Gravity is planned, and no provider status is faked.',
  },
  {
    id: 'pr-review',
    title: 'PR Review',
  ...
  },
  {
    ...
    prompt: 'Run the Content Factory playbook. Turn this idea into: blog post, short video script, LinkedIn post, X/Twitter post, thumbnail concept, newsletter blurb, and daily journal entry. Use Hermes only if it is configured through BertOS backend routes and any risky/paid tool usage was explicitly approved. Treat Hyperframes/Remotion as planned setup prompts only. Do not auto-post.',
  },
  {
    id: 'hermes-power-agent',
    title: 'Hermes Power Agent Upgrade',
    category: 'Hermes',
    bestAgent: 'BertOS Orchestrator, then Codex/Claude/Gemini as needed',
    risk: 'review-required',
    requiredTools: ['Memory', 'Prompts', 'Agents', 'Brief', 'Playbooks', 'optional verified connectors'],
    description: 'Upgrade Hermes-style operation through memory, super goals, model routing, swarms, content factory, video setup, and backup planning without fake integrations.',
    steps: [
      'Draft or update soul.md without secrets',
      'Create a Super Goal plan with human-agent handoffs',
      'Prepare the Morning Dream Brief prompt',
      'Assign models/tools by job and cost',
      'Turn the mission into Kanban agent tasks',
      'Use source-grounded content factory outputs when relevant',
      'Keep HyperFrames, NotebookLM MCP, Firecrawl, email, meetings, and GitHub backup setup-gated until verified',
      'Run validation before any code change is considered done',
    ],
    prompt: 'Run the Hermes Power Agent Upgrade. Build a memory-first operating plan with soul.md, Super Goal tasks, Morning Dream Brief, model routing, Kanban swarm roles, source-grounded content factory, HyperFrames setup checklist, and private backup plan. Keep Hermes server-side and setup-gated, treat unverified connectors as setup-gated, do not read secrets, do not auto-publish, do not push.',
  },
  {
    id: 'source-grounded-research-agent',
    title: 'Source-Grounded Research Agent',
    category: 'Research',
    bestAgent: 'Gemini, Claude, Codex, or Hermes only if research tools are configured and approved',
    risk: 'safe',
    requiredTools: ['source URLs', 'optional Firecrawl/search connector', 'memory note'],
    description: 'Use current web/source material for a grounded recommendation instead of relying on model memory.',
    steps: [
      'List the question and decision to make',
      'Collect source URLs or mark search unavailable',
      'Separate verified facts from assumptions',
      'Summarize implications for BertOS',
      'Recommend one implementation step',
      'Record the result in Memory or a session note',
    ],
    prompt: 'Run a source-grounded research pass. Use provided URLs or configured search tools, cite checked sources, separate verified facts from assumptions, recommend one BertOS implementation step, and do not claim unavailable connector access.',
  },
]
