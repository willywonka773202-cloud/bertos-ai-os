...
export async function POST(req: NextRequest) {
  ...
  if (!botToken || !allowedChatId) {
    return NextResponse.json({
      ok: false,
      error: 'Telegram is not configured. Set TELEGRAM_BOT_TOKEN and TELEGRAM_ALLOWED_CHAT_ID in .env.local.',
    }, { status: 503 })
  }
  ...
  try {
    const res = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
      ...
      body: JSON.stringify({ chat_id: allowedChatId, text: body.text, parse_mode: 'Markdown' }),
